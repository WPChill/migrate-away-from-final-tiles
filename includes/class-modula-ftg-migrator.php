...
class Modula_FTG_Migrator {
	...
	public function final_tiles_gallery_import( $gallery_id = '' ) {
		...
		if ( empty( $gallery_id ) ) {
			...
			if ( ! isset( $_POST['id'] ) ) {
				$this->modula_import_result( false, esc_html__( 'No gallery was selected', 'modula-final-tiles-migrator' ), false );
			}
		...
		if ( isset( $imported_galleries['galleries']['final_tiles'][ $gallery_id ] ) ) {
			...
			if ( 'modula-gallery' === $modula_gallery ) {
				...
				$this->modula_import_result( false, esc_html__( 'Gallery already migrated!', 'modula-final-tiles-migrator' ), false );
			}
		}
		...
		if ( count( $modula_images ) == 0 ) {
			...
			$this->modula_import_result( false, esc_html__( 'No images found in gallery. Skipping gallery...', 'modula-final-tiles-migrator' ), false );
		}
